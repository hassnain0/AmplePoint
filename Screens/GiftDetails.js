...
const GiftDetails = ({ navigation }) => {
  ...
  const selectedImage = imageData ? imageData[selectedImageIndex] : null;
  ...
  useEffect(() => {
    ...
    const getProductDetails = async () => {
      try {
        ...
        if (response.data && response.data.data.gift_card_detail_available === 1) {
            ...
            <View >
              ...
              <View style={{ flex: 1, flexDirection: 'row' }}>
                ...
                {imageData && imageData.map((image, index) => (
                  ...
                  <TouchableOpacity key={index} onPress={() => handleImageClick(index)} style={{ margin: moderateScale(5), borderWidth: selectedImageIndex === index ? 2 : 0, borderRadius: 10, borderColor: '#ff3d00' }}>
                    <Image
              ...
              <View style={styles.ShareContainer}>
                <TouchableOpacity onPress={handleShare} style={{ backgroundColor: 'transparent', padding: moderateScale(10), }}>
                  <Image source={require('../assets/Share.png')} style={{ width: Metrics.ratio(20), height: Metrics.ratio(20) }} />
                </TouchableOpacity>

              </View>
              <View>
                {selectedImage && (
                  <View style={{ flex: 1, alignContent: 'center', justifyContent: 'center', alignItems: 'center', padding: scale(10) }} >
                    <Image
                      style={styles.ImageContainer2} // Adjust the style as needed
                      source={{ uri: `https://amplepoints.com/product_images/${selectedImage.pid}/${selectedImage.pimage}` }}
                    />
                  </View>
                )}
              </View>
              ...
              <Text style={{
                color: 'black', fontWeight: '900', paddingLeft: Metrics.ratio(10), fontSize: 12, fontFamily: Platform.select({
                  ios: 'Times New Roman',
                  android: 'Times New Roman', // You may need to adjust this for Android
                }), bottom: Metrics.ratio(20), top: Metrics.ratio(10)
              }}>Apply Ample</Text>
                ...
                <RadioButton.Group onValueChange={showShippingDetails} value={isShippingSelected.toString()}>
                  {pickUp && (
                    <RadioButton.Item
                      ...
                      color='#FF2E00'
                      labelStyle={{
                        fontSize: 12, fontFamily: Platform.select({
                          ios: 'Times New Roman',
                  ...
                  <RadioButton.Group onValueChange={showShippingDetails} value={isShippingSelected.toString()}>
                    {shipping && (
                      <RadioButton.Item
                        ...
                        color='#FF2E00'
                        labelStyle={{
                          fontSize: 10, fontFamily: Platform.select({
                            ios: 'Times New Roman',
                            android: 'Arial', // You may need to adjust this for Android
                          }),
                        }}
