...
const CustomDrawerContent = ({ navigation, CompleteProfile, ...props }) => {
  const removeItem = async () => {
    await AsyncStorage.removeItem("userLoggedIn");
    util.successMsg("Logout Sucessfully");
    // props.navigation.navigate('Login'); // Replace 'Login' with your actual login screen name
  ...
  return (
    <DrawerContentScrollView {...props}>
      ...
      <DrawerItem
        label="Logout"
        icon={({ color, size }) => <Image source={require('./assets/logout.png')} style={{ width: 15, height: 15, tintColor: color }} />}
        onPress={removeItem}
  ...
  return (
    <Drawer.Navigator drawerContent={(props) => <CustomDrawerContent {...props} CompleteProfile={CompleteProfile} />} screenOptions={{
      ...
      <Drawer.Screen name="Home" initialParams={CompleteProfile1 = CompleteProfile} component={TabNavigator} options={{
        drawerIcon: ({ color }) => (
          <Image source={require('./assets/home1.png')} style={{ width: 15, height: 15 }} />
        ),
        drawerLabelStyle: {
          fontSize: 10,
          fontWeight: '700',
      ...
      <Drawer.Screen name="MyPurchase" component={MyPurchase} initialParams={{ Profile: route.params?.CompleteProfile.user_id || {} }} options={{
        headerShown: false, drawerIcon: ({ color }) => (
          <Image source={require('./assets/Purchase.png')} style={{ width: 15, height: 15 }} />
        ), drawerLabelStyle: {
          fontSize: 10,
          fontFamily: 'Arial',
          fontWeight: '700',
      ...
      <Drawer.Screen name="LocalPurchase" component={LocalPurchase} options={{
        headerShown: false, drawerIcon: ({ color }) => (
          <Image source={require('./assets/Purchase.png')} style={{ width: 15, height: 15 }} />
        ), drawerLabelStyle: {
          fontSize: 10,
